...
import type { CreateInvoiceInput } from '@/api/invoices';

type Mode = 'DERRAMA' | 'INDIVIDUAL';

export function CreateInvoicePage(): JSX.Element {
  ...
  const [mode, setMode] = useState<Mode>('DERRAMA');
  ...
  // INDIVIDUAL: importe por unidad
  const [itemAmounts, setItemAmounts] = useState<Record<string, string>>({});
  const [itemConsumption, setItemConsumption] = useState<Record<string, string>>({});

  // Previsualización del reparto de derrama por coeficiente
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-8">
        ...
        <section className="card">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {(['DERRAMA', 'INDIVIDUAL'] as Mode[]).map((m) => (
              <button
                type="button"
                key={m}
        ...
        <section className="card space-y-5">
          <div>
            ...
            <input id="concept" className="input" value={concept} onChange={(e): void => setConcept(e.target.value)} required placeholder={t('invoices.conceptPlaceholder')} />
          </div>
          <div>
            <label className="label" htmlFor="description">{t('invoices.description')}</label>
            <textarea id="description" className="input min-h-20" value={description} onChange={(e): void => setDescription(e.target.value)} />
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className="label" htmlFor="dueDate">{t('invoices.dueDate')}</label>
              <input id="dueDate" type="date" className="input" value={dueDate} onChange={(e): void => setDueDate(e.target.value)} required />
            </div>
            <div>
              <label className="label" htmlFor="attachmentUrl">{t('invoices.attachmentUrl')}</label>
              <input id="attachmentUrl" type="url" className="input" value={attachmentUrl} onChange={(e): void => setAttachmentUrl(e.target.value)} placeholder="https://…" />
            </div>
          </div>
        </section>

        {/* DERRAMA: total + previsualización del reparto */}
        {mode === 'DERRAMA' && (
          <section className="card">
            ...
            <div className="mt-4 max-w-xs">
              ...
              <input id="totalAmount" type="number" step="0.01" min="0" className="input font-mono" value={totalAmount} onChange={(e): void => setTotalAmount(e.target.value)} required />
            </div>
                    ...
                    <tr key={u.id} className="border-b border-olive-50">
                      ...
                      <td className="py-2 pr-3">
                        <input
                          ...
                          onChange={(e): void => setItemAmounts((prev) => ({ ...prev, [u.id]: e.target.value }))}
                          placeholder="0.00"
                        />
                      </td>
                      <td className="py-2 text-right">
                        <input
                          ...
                          onChange={(e): void => setItemConsumption((prev) => ({ ...prev, [u.id]: e.target.value }))}
                          placeholder="—"
                        />
                      </td>
                    </tr>
