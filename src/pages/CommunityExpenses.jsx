...
export function CommunityExpensesPage() {
        ...
        <form onSubmit={onSubmit} className="card mt-6 space-y-5">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input id="amount" type="number" step="0.01" min="0" className="input font-mono" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} required />
            </div>
          </div>
          <div>
            <label className="label" htmlFor="concept">{t('expenses.concept')}</label>
            <input id="concept" className="input" value={form.concept} onChange={(e) => setForm({ ...form, concept: e.target.value })} required placeholder={t('expenses.conceptPlaceholder')} />
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              <label className="label" htmlFor="expenseDate">{t('expenses.date')}</label>
              <input id="expenseDate" type="date" className="input" value={form.expenseDate} onChange={(e) => setForm({ ...form, expenseDate: e.target.value })} required />
            </div>
            <div>
              <label className="label" htmlFor="supplier">{t('expenses.supplier')}</label>
              <input id="supplier" className="input" value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} />
            </div>
          </div>
          <div>
            <label className="label" htmlFor="attachmentUrl">{t('expenses.attachmentUrl')}</label>
            <input id="attachmentUrl" type="url" className="input" value={form.attachmentUrl} onChange={(e) => setForm({ ...form, attachmentUrl: e.target.value })} placeholder="https://…" />
          </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_340px]">
        <div>
          <div className="mb-3 flex items-center gap-2">
            <select className="input max-w-xs py-1.5 text-sm" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
                  ...
                  {data.expenses.map((ex) => (
                    <tr key={ex.id} className="border-b border-olive-50 last:border-0">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => handleDelete(ex.id)} className="text-xs text-olive-500 hover:text-clay-600">✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
