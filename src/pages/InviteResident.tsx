...
export function InviteResidentPage(): JSX.Element {
  ...
  } = useForm<FormValues>({ defaultValues: { relationType: 'OWNER' } });
  ...
  // Cuando cambia la comunidad, limpiar unidad seleccionada
  useEffect(() => {
    setValue('unitId', '');
  ...
  const usedUnitIds = useMemo(() => {
    ...
    units.forEach((u) => {
      // Si ya tiene un occupant activo, marcamos para indicarlo en el UI
  ...
  const onSubmit = async (values: FormValues): Promise<void> => {
    ...
    try {
      const res = await invitationsApi.createInvitation({
        ...values,
        locale: (i18n.resolvedLanguage as 'es' | 'en') ?? 'es',
      });
      setFeedback({ kind: 'ok', msg: t('invite.success', { email: res.sentTo }) });
      reset({ relationType: 'OWNER' });
    } catch (err) {
      const apiErr = err as { response?: { data?: { error?: { message?: string } } } };
      setFeedback({ kind: 'err', msg: apiErr.response?.data?.error?.message ?? t('errors.generic') });
    }
  };
    ...
    <Layout>
      ...
      {!noCommunities && (
        <form onSubmit={handleSubmit(onSubmit)} className="card mt-8 max-w-2xl space-y-5" noValidate>
          ...
          {/* Selectores reales encadenados */}
          <div>
            <label className="label" htmlFor="communityId">
              {t('invite.community')}
            </label>
            <select
              id="communityId"
          ...
          <div>
            ...
            <select
              id="unitId"
              className="input"
              disabled={!selectedCommunityId || loadingUnits}
