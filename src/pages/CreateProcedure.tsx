...
export function CreateProcedurePage(): JSX.Element {
  ...
  const [type, setType] = useState<ProcedureType>('CERTIFICATE');
      ...
      <form onSubmit={onSubmit} className="card mt-8 max-w-2xl space-y-5">
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            ...
            <select id="community" className="input" value={communityId} onChange={(e): void => setCommunityId(e.target.value)} required>
              {communities?.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </div>
          <div>
            <label className="label" htmlFor="type">{t('procedures.type')}</label>
            <select id="type" className="input" value={type} onChange={(e): void => setType(e.target.value as ProcedureType)}>
        ...
        <div>
          ...
          <input id="subject" className="input" value={subject} onChange={(e): void => setSubject(e.target.value)} required maxLength={200} />
        </div>
        <div>
          <label className="label" htmlFor="description">{t('procedures.description')}</label>
          <textarea id="description" className="input min-h-32" value={description} onChange={(e): void => setDescription(e.target.value)} required placeholder={t('procedures.descriptionPlaceholder')} />
        </div>
