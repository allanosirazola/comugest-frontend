        ...
        <div>
          ...
          {procedure.status !== 'COMPLETED' && procedure.status !== 'REJECTED' && (
            <form onSubmit={onMessage} className="card mt-4 space-y-3">
              <textarea className="input min-h-20" value={message} onChange={(e) => setMessage(e.target.value)} placeholder={t('procedures.messagePlaceholder')} />
        ...
        {canManage && (
          <aside className="space-y-4">
            <div className="card space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => void updateProcedure.mutate({ status: e.target.value })}
                >
            ...
            <div className="card space-y-3">
              ...
              <textarea className="input min-h-20" value={resolution} onChange={(e) => setResolution(e.target.value)} placeholder={t('procedures.resolutionPlaceholder')} />
              <div>
                <label className="label">{t('procedures.documentUrl')}</label>
                <input type="url" className="input" value={attachmentUrl} onChange={(e) => setAttachmentUrl(e.target.value)} placeholder="https://…" />
              </div>
