...
import { useMyCalendar } from '@/hooks/useCalendar';
...
export function MyCalendarPage() {
  ...
  const { data: events = [], isLoading } = useMyCalendar(params);
    ...
    <Layout>
      ...
      {isLoading ? (
                        ...
                        <>
                          <span
                            className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-medium ${
                              isToday ? 'bg-olive-700 text-cream-50' : 'text-olive-800'
                            }`}
                          >
                            {day}
                          </span>
        ...
        Object.keys(byDay).length === 0 ? (
          <p className="mt-10 text-center text-olive-500">{t('calendar.empty')}</p>
        ) : (
              ...
              <div key={day}>
                ...
                <div className="space-y-2">
                  {dayEvents.map((ev) => {
                    ...
                    return (
                      <div key={ev.id} className={`flex items-start gap-3 rounded-lg border px-3 py-2 ${cfg.color}`}>
                        <span className="mt-0.5 shrink-0 text-[10px] font-medium uppercase tracking-wider opacity-70">{t(cfg.label)}</span>
                        <div className="min-w-0 flex-1">
                          <p className="font-medium">{ev.title}</p>
                          {ev.endDate && (
                            <p className="text-xs opacity-70">
                              {new Date(ev.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} → {new Date(ev.endDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </p>
                          )}
                        </div>
                      </div>
