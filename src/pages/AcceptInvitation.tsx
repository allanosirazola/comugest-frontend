...
import { acceptInvitationSchema, type AcceptInvitationValues } from '@/lib/schemas';
...
export function AcceptInvitationPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setLoadError(apiErr.response?.data?.error?.message ?? t('auth.acceptInvitation.invalidToken'));
      }
    })();
    return (): void => {
      cancelled = true;
  ...
  const onSubmit = async (values: AcceptInvitationValues): Promise<void> => {
    if (!token) return;
    setServerError(null);
    try {
      ...
      const apiErr = err as { response?: { data?: { error?: { message?: string } } } };
      setServerError(apiErr.response?.data?.error?.message ?? t('errors.generic'));
    }
  };
