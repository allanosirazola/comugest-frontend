...
export function CreateCommunityPage(): JSX.Element {
  ...
  const addUnit = (): void => setUnits((prev) => [...prev, emptyUnit()]);
  const removeUnit = (key: string): void => setUnits((prev) => prev.filter((u) => u._key !== key));
  const updateUnit = (key: string, patch: Partial<DraftUnit>): void => {
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-8">
        ...
        <section className="card space-y-5">
          ...
          <div>
            ...
            <input id="cif" className="input" value={cif} onChange={(e): void => setCif(e.target.value)} />
          </div>
        </section>
            ...
            <div className="text-right text-xs">
              ...
              <p
                className={`font-mono text-lg ${
                  coefficientSum > 100.01 ? 'text-clay-600' : 'text-olive-800'
                }`}
              >
                  ...
                  <tr key={u._key} className="border-b border-olive-50">
                    <td className="py-2 pr-3">
                      <select
                        ...
                        value={u.type}
                        onChange={(e): void => updateUnit(u._key, { type: e.target.value as UnitType })}
                      >
                    ...
                    <td className="py-2 pr-3">
                      <input
                        ...
                        placeholder="3ºB"
                        value={u.label}
                        onChange={(e): void => updateUnit(u._key, { label: e.target.value })}
                      />
                    </td>
                    <td className="py-2 pr-3">
                      <input
                        ...
                        value={u.floor}
                        onChange={(e): void => updateUnit(u._key, { floor: e.target.value })}
                      />
                    </td>
                    <td className="py-2 pr-3">
                      <input
                        className="input py-1.5 w-20"
                        value={u.door}
                        onChange={(e): void => updateUnit(u._key, { door: e.target.value })}
                      />
                    </td>
                    <td className="py-2 pr-3">
                      <input
                        className="input py-1.5 w-24 text-right font-mono"
                        type="number"
                        step="0.01"
                        min="0"
                        max="100"
                        value={u.coefficient}
                        onChange={(e): void => updateUnit(u._key, { coefficient: e.target.value })}
                      />
                    </td>
                    <td className="py-2 pr-3">
                      <input
                        ...
                        min="0"
                        value={u.surfaceM2}
                        onChange={(e): void => updateUnit(u._key, { surfaceM2: e.target.value })}
                      />
                    </td>
                    <td className="py-2 text-right">
                      <button
                        type="button"
                        onClick={(): void => removeUnit(u._key)}
