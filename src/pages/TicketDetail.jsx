        ...
        <div>
          ...
          {ticket.status !== 'CLOSED' && (
            <form onSubmit={onComment} className="card mt-4 space-y-3">
              <textarea className="input min-h-20" value={comment} onChange={(e) => setComment(e.target.value)} placeholder={t('tickets.commentPlaceholder')} />
              <div className="flex items-center justify-between">
                {isSupport && (
                  <label className="flex items-center gap-2 text-xs text-olive-600">
                    <input type="checkbox" className="h-4 w-4 rounded border-olive-300 text-olive-700" checked={internal} onChange={(e) => setInternal(e.target.checked)} />
            ...
            <div className="card space-y-4">
              ...
              <div>
                ...
                <select
                  className="input"
                  value={ticket.status}
                  onChange={(e) => void updateTicket.mutate({ status: e.target.value })}
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => void updateTicket.mutate({ priority: e.target.value })}
                >
