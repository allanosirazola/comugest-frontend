...
export function MyExpensesPage(): JSX.Element {
  const { t } = useTranslation();
  const { data: communities } = useMyCommunities();
  const [communityId, setCommunityId] = useState<string>('');

  // Seleccionar la primera comunidad por defecto
  useEffect(() => {
    ...
    <Layout>
      ...
      {communities && communities.length > 1 && (
        <div className="mt-6 max-w-xs">
          ...
          <select className="input" value={communityId} onChange={(e): void => setCommunityId(e.target.value)}>
            {communities.map((c) => (
